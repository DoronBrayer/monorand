...
export function testBasicDoubleGeneration(): void {
    ...
    try {
        ...
        const num = cryptoRandom({ lowerBound: lowerBound, upperBound: upperBound, typeOfNum: 'double', maxFracDigits: maxFracDigits });
        assert(num >= lowerBound && num <= upperBound, `[${testID}] Failed: Expected double in [${lowerBound},${upperBound}], got ${num}`);
        const decimalPlaces = num.toString().includes('.') ? num.toString().split('.')[1].length : 0;
        assert(decimalPlaces <= maxFracDigits, `[${testID}] Failed: Expected ${maxFracDigits} frac digits, got ${decimalPlaces} for ${num}`);
        console.log(`[${testID}] Passed: Double in specific range [${lowerBound},${upperBound}] with ${maxFracDigits} frac digits. Result: ${num}`);
    } catch (e: any) {
    ...
    try {
        ...
        assert(decimalPlaces <= maxFracDigits, `[${testID}] Failed: Expected <= ${maxFracDigits} decimal places, got ${decimalPlaces} for ${actualYield}`);
        console.log(`[${testID}] Passed: Fractional digits for double (maxFracDigits: ${maxFracDigits}). Result: ${actualYield}`);
    } catch (e: any) {
    ...
    try {
        ...
        const decimalPlaces = actualYield.toString().includes('.') ? actualYield.toString().split('.')[1].length : 0;
        assert(decimalPlaces <= maxFracDigits, `[${testID}] Failed: Expected <= ${maxFracDigits} decimal places, got ${decimalPlaces} for ${actualYield}`);
        console.log(`[${testID}] Passed: Different Max Fractional Digits (maxFracDigits: ${maxFracDigits}). Result: ${actualYield}`);
    } catch (e: any) {
    ...
    try {
        ...
        const actualQtyOfFractionalDigits = actualYield.toString().includes('.') ? actualYield.toString().split('.')[1].length : 0;
        // We can't guarantee exactly 15, but it should be <= 15 and greater than 0 for a random double
        assert(actualQtyOfFractionalDigits <= expectedQtyOfFractionalDigits, `[${testID}] Failed: Expected <= ${expectedQtyOfFractionalDigits} frac digits, got ${actualQtyOfFractionalDigits}`);
        // Also check it's not an integer if maxFracDigits > 0
        if (expectedQtyOfFractionalDigits > 0) {
            assert(actualQtyOfFractionalDigits > 0 || actualYield === 0 || actualYield === 1, `[${testID}] Failed: Expected fractional part, got integer for ${actualYield}`);
        }
        console.log(`[${testID}] Passed: Ultra-high maxFracDigits. Result: ${actualYield} (${actualQtyOfFractionalDigits} digits)`);
    } catch (e: any) {
    ...
    try {
        ...
        console.log(`[${testID}] Passed: Twelve fractional digits. Result: ${actualYield} (${actualQtyOfFractionalDigits} digits)`);
    } catch (e: any) {
    ...
    try {
        ...
        const decimalPlaces = actualYield.toString().includes('.') ? actualYield.toString().split('.')[1].length : 0;
        assert.strictEqual(decimalPlaces, 0, `[${testID}] Failed: Expected 0 decimal places, got ${decimalPlaces} for ${actualYield}`);
