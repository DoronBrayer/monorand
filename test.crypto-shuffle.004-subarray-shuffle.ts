...
describe('cryptoShuffle: Subarray Shuffle', () => {
    ...
    it('Should throw TypeError for invalid startIndex type', () => {
        const originalArray = [1, 2, 3]
        expect(() => cryptoShuffle(originalArray, { startIndex: '1' as any })).toThrow(TypeError)
        expect(() => cryptoShuffle(originalArray, { startIndex: 1.5 })).toThrow(
            /Invalid cryptoShuffle parameters: startIndex.*must be.*integer/
        )
    })

    it('Should throw TypeError for invalid endIndex type', () => {
        const originalArray = [1, 2, 3]
        expect(() => cryptoShuffle(originalArray, { endIndex: '3' as any })).toThrow(TypeError)
        expect(() => cryptoShuffle(originalArray, { endIndex: 3.5 })).toThrow(
            /Invalid cryptoShuffle parameters: endIndex.*must be.*integer/
        )
    })

    it('Should throw TypeError for startIndex out of bounds (too high)', () => {
        const originalArray = [1, 2, 3] // length 3, valid indices 0,1,2,3 (end)
        expect(() => cryptoShuffle(originalArray, { startIndex: 4 })).toThrow(
            /Invalid cryptoShuffle parameters: 'startIndex' \(4\) must be between 0 and the array length \(3\)/
        )
    })

    it('Should throw TypeError for endIndex out of bounds (too high)', () => {
        const originalArray = [1, 2, 3] // length 3, valid indices 0,1,2,3 (end)
        expect(() => cryptoShuffle(originalArray, { endIndex: 4 })).toThrow(
            /Invalid cryptoShuffle parameters: 'endIndex' \(4\) must be between 0 and the array length \(3\)/
        )
    })

    it('Should throw TypeError for startIndex out of bounds (negative)', () => {
        const originalArray = [1, 2, 3]
        expect(() => cryptoShuffle(originalArray, { startIndex: -1 })).toThrow(
            /Invalid cryptoShuffle parameters: 'startIndex' \(-1\) must be between 0 and the array length \(3\)/
        )
    })

    it('Should throw TypeError for endIndex out of bounds (negative)', () => {
        const originalArray = [1, 2, 3]
        expect(() => cryptoShuffle(originalArray, { endIndex: -1 })).toThrow(
            /Invalid cryptoShuffle parameters: 'endIndex' \(-1\) must be between 0 and the array length \(3\)/
        )
    })
})
