...
test('cryptoRandom: Core Functionality & Basic Parameters', async (t) => {
    // 't' is the TestContext for nesting
    ...
    await t.test("CRTC06: Single Parameter: exclusion (Integer - 'both')", () => {
        ...
        assert(result === 1, `[${testID}] Failed: Expected integer 1 with 'both' exclusion on [0, 2], got ${result}`)
