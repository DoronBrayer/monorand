...
export function testBasicIntegerGeneration(): void {
    ...
    try {
        ...
        assert.strictEqual(actualYield, expectedYield, `[${testID}] Failed: Expected ${expectedYield}, got ${actualYield}`);
    ...
    try {
        ...
        assert(num >= lowerBound && num <= upperBound, `[${testID}] Failed: Expected integer in [${lowerBound},${upperBound}], got ${num}`);
